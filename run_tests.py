    ...
    for filename in txt_filenames:
        ...
        with open(path) as f:
            for l in f:
                ...
                (lib, status, json_filename) = (
                    comps[0], comps[1], comps[2].rstrip())
...
def generate_report(report_path, keep_only_first_result_in_set=False):
    ...
    tests_with_same_results = f_tests_with_same_results(
        libs, status_for_lib_for_file)
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        for (k, file_set) in tests_with_same_results:
            ...
            for path in [path for path in ordered_file_set if os.path.exists(path)]:
                ...
                f.write('        <TD>%s</TD>' %
                        f_underline_non_printable_bytes(bytes))
                f.write("    </TR>")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--filter', dest='restrict_to_program',
    ...
    generate_report(os.path.join(BASE_DIR, "results/parsing.html"),
                    keep_only_first_result_in_set=False)
    generate_report(os.path.join(
        BASE_DIR, "results/parsing_pruned.html"), keep_only_first_result_in_set=True)
